...
def train_sdt4_multi(input_Var, target_Var, encoders, decoders, enc_opts, dec_opts, 
    ...
    for step in range((input_time_step//BATCH_SIZE)+1):
        if BATCH_SIZE*step > k and BATCH_SIZE*step < input_time_step - (k+1) - BATCH_SIZE:
            # Sampling
            if target_Var[:, BATCH_SIZE*step, 2].item() != 1.0:
                r = random.random()
                if r < 0.5:
                    continue
            ...
            for i in range(BATCH_SIZE):
                onLoss += onLossFunc(onDecOut[i].view(1, OUTPUT_SIZE), target_Var[:,BATCH_SIZE*step+i].contiguous().view(1, OUTPUT_SIZE))
